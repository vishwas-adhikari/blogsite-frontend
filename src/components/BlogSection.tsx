...
import { Link } from 'react-router-dom'; // REMOVED: useNavigate, as <Link> handles it
import { Calendar, Clock, ExternalLink, Search, Filter } from 'lucide-react';
import { fetchBlogPosts } from '../services/api';

// This interface now perfectly matches the data from your Django API
interface BlogPost {
  id: number;
  title: string;
  slug: string;
...
const BlogSection: React.FC = () => {
  ...
  useEffect(() => {
    fetchBlogPosts()
      // FIXED: We now tell TypeScript that 'data' is an array of BlogPost objects
      .then((data: BlogPost[]) => {
        setAllPosts(data);
      ...
      // FIXED: We handle the error type correctly
      .catch((err: unknown) => {
        console.error("Failed to fetch blog posts:", err);
        // This safely converts the error to a string for display
  ...
  // --- JSX REMAINS THE SAME, BUT NOW USES CORRECT VARIABLE NAMES ---
  return (
